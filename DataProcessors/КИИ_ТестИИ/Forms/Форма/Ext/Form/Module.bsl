// Промпт для отложенного синхронного запроса (чтобы успела отрисоваться плашка ожидания)
Перем ПромптОтложенногоСинхронногоЗапроса;
...
Процедура ВыполнитьОтложенныйСинхронныйЗапросКМодели()
	...
	ПромптОтложенногоСинхронногоЗапроса = Неопределено;
...
Функция СформироватьСтруктураПромпта(Знач Эксперт, Знач Промпт)
	...
	Результат.СистемныйПромпт = ПромптЭксперта + КИИ_КоннекторИИ.КонтекстТекущегоПользователя();
