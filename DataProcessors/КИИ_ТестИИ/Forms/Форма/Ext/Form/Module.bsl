...
&НаКлиенте
Процедура ОтветHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Позиция = Найти(ИмяОткрываемойСтраницы , "e1cib/");
	Если Позиция > 0 Тогда
		ПерейтиПоНавигационнойСсылке(Сред(ИмяОткрываемойСтраницы , Позиция));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗапросСинхронно(ТекущийПромпт)
	...
	ПоказатьПлашкуРазмышленияНаСервере();
	
	ПромптОтложенногоСинхронногоЗапроса = ТекущийПромпт;
	ПодключитьОбработчикОжидания("ВыполнитьОтложенныйСинхронныйЗапросКМодели", 0.05, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьОтложенныйСинхронныйЗапросКМодели()
	
	ТекущийПромпт = ПромптОтложенногоСинхронногоЗапроса;
	ПромптОтложенногоСинхронногоЗапроса = "";
	ЗаголовкиОткрытыхОкон = ПолучитьЗаголовкиОткрытыхОкон();
	
	Попытка
		РезультатЗапроса = ВыполнитьЗапросНаСервере(ТекущийПромпт, ЗаголовкиОткрытыхОкон);
	Исключение
		УбратьПлашкуРазмышленияНаСервере();
...
&НаСервере
Функция ВыполнитьЗапросНаСервере(Знач ТекстПромпта, Знач ЗаголовкиОткрытыхОкон = Неопределено)

	Модель = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Эксперт, "Модель");
	СтруктураПромпта = СформироватьСтруктураПромпта(Эксперт, ТекстПромпта, ЗаголовкиОткрытыхОкон);

	Инструменты = КИИ_КоннекторИИ.ИнструментыЭксперта(Эксперт);
	Если Инструменты.Количество() > 0 Тогда
		Результат = КИИ_КоннекторИИ.ЗапросКМоделиСИнструментами(Модель, СтруктураПромпта, Инструменты);
	Иначе
...
Процедура ЗапуститьЗапросВФоне(ТекстПромпта)
	...
	Попытка
		ФоновоеЗадание = ЗапуститьЗапросВФонеНаСервере(ТекстПромпта, ЗаголовкиОткрытыхОкон);
	Исключение
		УбратьПлашкуРазмышленияНаСервере();
...
&НаСервере
Функция ЗапуститьЗапросВФонеНаСервере(Знач ТекстПромпта, Знач ЗаголовкиОткрытыхОкон = Неопределено)
	...
	Инструменты = КИИ_КоннекторИИ.ИнструментыЭксперта(Эксперт);
	Если Инструменты.Количество() > 0 Тогда
		Возврат КИИ_КоннекторИИ.ЗапросКМоделиСИнструментамиВФоне(Модель, СтруктураПромпта, Инструменты);
	КонецЕсли;
...
&НаСервере
Процедура ПоказатьПлашкуРазмышленияНаСервере()
	
	УбратьПлашкуРазмышленияНаСервере();
	
	СтрокаРазмышления = ИсторияДиалога.Добавить();
	СтрокаРазмышления.Роль = "thinking";
	СтрокаРазмышления.Текст = НСтр("ru = 'Агент размышляет…'");
	СтрокаРазмышления.Метаинформация = "";
...
&НаСервере
Процедура УбратьПлашкуРазмышленияНаСервере()
	
	Пока ИсторияДиалога.Количество() > 0
		И ИсторияДиалога[ИсторияДиалога.Количество() - 1].Роль = "thinking" Цикл
		
		ИсторияДиалога.Удалить(ИсторияДиалога.Количество() - 1);
...
Процедура ОбновитьОтображениеОтвета()
	...
	Шаблон = Обработки.КИИ_ТестИИ.ПолучитьМакет("ШаблонДиалога").ПолучитьТекст();
		...
		Для Каждого Сообщение Из ИсторияДиалога Цикл
			СообщенияHTML = СообщенияHTML + СформироватьHTMLСообщения(Сообщение, ИмяПользователя);
		КонецЦикла;
...
&НаСервере
Функция СформироватьHTMLСообщения(Сообщение, ИмяПользователя)
	...
	ИначеЕсли Сообщение.Роль = "assistant" Тогда
		...
		Эмодзи = "⚒"; 
	ИначеЕсли Сообщение.Роль = "thinking" Тогда
		Класс = "thinking";
		ИмяРоли = НСтр("ru = 'Ожидание'");
		Эмодзи = "⏳"; 
	Иначе 
		Класс = "error";
	...
	Если Сообщение.Роль = "thinking" Тогда
		ТекстСообщения = КИИ_МаркдаунПарсерКлиентСервер.ЭкранироватьHTML(Сообщение.Текст);
	Иначе
...
&НаКлиенте
Функция ПолучитьЗаголовкиОткрытыхОкон()
	...
	Для Каждого ОкноПриложения Из СписокОконПриложения Цикл
		...
		Заголовок = СокрЛП(ОкноПриложения.Заголовок);
		Если ПустаяСтрока(Заголовок) Тогда
			Продолжить;
		КонецЕсли;
		
		Если Заголовки.Найти(Заголовок) = Неопределено Тогда
...
&НаСервере
Функция СформироватьСтруктураПромпта(Знач Эксперт, Знач Промпт, Знач ЗаголовкиОткрытыхОкон = Неопределено)
	...
	Результат.СистемныйПромпт = ПромптЭксперта + КИИ_КоннекторИИ.КонтекстТекущегоПользователя(ЗаголовкиОткрытыхОкон);
	...
	Если ИсторияДиалога.Количество() > 1 Тогда
		...
		ИндексПоследнегоВКонтексте = ИсторияДиалога.Количество() - 1;
		Пока ИндексПоследнегоВКонтексте >= 0
			И (ИсторияДиалога[ИндексПоследнегоВКонтексте].Роль = "thinking") Цикл
			
			ИндексПоследнегоВКонтексте = ИндексПоследнегоВКонтексте - 1;
		...
		Для Индекс = НачальнаяПозиция По ИсторияДиалога.Количество() - 1 Цикл
			...
			Если Сообщение.Роль = "error" Или Сообщение.Роль = "thinking" Тогда
				Продолжить;
			КонецЕсли;
			
			Если Индекс = ИндексПоследнегоВКонтексте И Сообщение.Роль = "user" Тогда
				Продолжить;
			КонецЕсли;
