...
// Возвращает строку с контекстом текущего пользователя для добавления в системный промпт.
...
//
// Параметры:
//  ЗаголовкиОткрытыхОкон - Массив из Строка, Неопределено - заголовки окон, открытых пользователем.
//    Собираются на клиенте через ПолучитьОкна().
//
// Возвращаемое значение:
//  Строка - контекст пользователя
Функция КонтекстТекущегоПользователя(ЗаголовкиОткрытыхОкон = Неопределено) Экспорт

	ЧастиКонтекста = Новый Массив;

	ТекущийПользователь = ПолучитьТекущегоПользователя();
	Если ТекущийПользователь <> Неопределено Тогда
		ЧастиКонтекста.Добавить(СтрШаблон(
	...
	ТекстОткрытыхФорм = СформироватьТекстОткрытыхФорм(ЗаголовкиОткрытыхОкон);
	Если ЗначениеЗаполнено(ТекстОткрытыхФорм) Тогда
		ЧастиКонтекста.Добавить(ТекстОткрытыхФорм);
	КонецЕсли;

	Если ЧастиКонтекста.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;

	Возврат Символы.ПС + Символы.ПС + СтрСоединить(ЧастиКонтекста, Символы.ПС + Символы.ПС);

КонецФункции

// Формирует текстовое описание открытых форм пользователя для системного промпта.
//
// Параметры:
//  ЗаголовкиОткрытыхОкон - Массив из Строка, Неопределено
//
// Возвращаемое значение:
//  Строка
Функция СформироватьТекстОткрытыхФорм(ЗаголовкиОткрытыхОкон)

	Если ЗаголовкиОткрытыхОкон = Неопределено Или ЗаголовкиОткрытыхОкон.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;

	СтрокиЗаголовков = Новый Массив;
	Для Каждого Заголовок Из ЗаголовкиОткрытыхОкон Цикл
		...
		СтрокиЗаголовков.Добавить("- " + Заголовок);

	КонецЦикла;

	Если СтрокиЗаголовков.Количество() = 0 Тогда
		Возврат "";
	КонецЕсли;

	Возврат СтрШаблон(
		НСтр("ru = 'Открытые формы пользователя:%1%2'"),
		Символы.ПС,
		СтрСоединить(СтрокиЗаголовков, Символы.ПС));
...
Функция ПолучитьКлючAPI(Модель, Параметры) 
	...
	ТребуетсяПривилегированныйРежим = Ложь;
	Попытка
		ВыполнитьПроверкуПравДоступа("Чтение", Метаданные.РегистрыСведений.БезопасноеХранилищеДанных);
		ВыполнитьПроверкуПравДоступа("Чтение", Метаданные.РегистрыСведений.БезопасноеХранилищеДанныхОбластейДанных);
	Исключение
		ТребуетсяПривилегированныйРежим = Истина;
		УстановитьПривилегированныйРежим(ТребуетсяПривилегированныйРежим);
	КонецПопытки;
	...
	Попытка
		...
		Если ТребуетсяПривилегированныйРежим Тогда
	...
	Для Каждого Вызов Из ВызовыToolCalls Цикл
		...
		Если Вызов.Свойство("MCPСервер") И ЗначениеЗаполнено(Вызов.MCPСервер) Тогда
			...
			Попытка
				...
				РезультатВызова.Вставить("MCPСервер", Строка(Вызов.MCPСервер));
