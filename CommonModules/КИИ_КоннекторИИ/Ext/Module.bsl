...
Процедура ЗапросКМоделиСИнструментамиФоновоеЗадание(ПараметрыМетода, АдресРезультата) Экспорт
	...
	Если ПараметрыМетода.Свойство("ИмяОбработчикаИнструмента")
		...
		Обработчик = Новый ОписаниеОповещения(
			ПараметрыМетода.ИмяОбработчикаИнструмента, ЭтотОбъект);
	КонецЕсли;
...
Функция СформироватьСообщенияРезультатовИнструментовOpenAI(
	...
	Для Индекс = 0 По РезультатыВызовов.ВерхняяГраница() Цикл
...
Функция ВыполнитьВызовыИнструментов(ВызовыToolCalls, Обработчик, НомерИтерации)
	...
	Для Каждого Вызов Из ВызовыToolCalls Цикл
		...
		Если Вызов.Свойство("MCPСервер") И ЗначениеЗаполнено(Вызов.MCPСервер) Тогда
			...
			Попытка
				РезультатMCP = КИИ_ConнекторMCP.ВызватьИнструмент(
					Вызов.MCPСервер,
					Вызов.Имя,
...
// Вызывает обработчик инструмента через ОписаниеОповещения
//
Процедура ВыполнитьОбработчикИнструмента(Обработчик, Вызов, РезультатВызова)
	...
	СтруктураРезультата.Вставить("Данные", "");
	СтруктураРезультата.Вставить("ТекстОшибки", "");

	// Вызываем обработчик, передавая имя, аргументы и структуру для результата
	Попытка
		Обработчик.Выполнить(Вызов.Имя, Вызов.Аргументы, СтруктураРезультата);
	Исключение
		// Если Выполнить не поддерживается, пробуем через УведомитьОВыполнении
		Попытка
			ДополнительныеПараметры = Новый Структура;
			ДополнительныеПараметры.Вставить("Вызов", Вызов);
			ДополнительныеПараметры.Вставить("РезультатВызова", СтруктураРезультата);
			УведомитьОВыполнении(Обработчик, ДополнительныеПараметры);
		Исключение
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Не удалось вызвать обработчик инструмента %1: %2'"),
				Вызов.Имя,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	КонецПопытки;

	Если СтруктураРезультата.Свойство("Успешно") Тогда
		РезультатВызова.Вставить("Успешно", СтруктураРезультата.Успешно);
	Иначе
		РезультатВызова.Вставить("Успешно", Истина);
	КонецЕсли;

	Если СтруктураРезультата.Свойство("Данные") Тогда
		РезультатВызова.Вставить("Данные", СтруктураРезультата.Данные);
	Иначе
		РезультатВызова.Вставить("Данные", "");
	КонецЕсли;

	Если СтруктураРезультата.Свойство("ТекстОшибки") Тогда
		РезультатВызова.Вставить("ТекстОшибки", СтруктураРезультата.ТекстОшибки);
	Иначе
		РезультатВызова.Вставить("ТекстОшибки", "");
	КонецЕсли;
