...
// Возвращает строку с информацией о текущем пользователе и его группах доступа
// для добавления в системный промпт.
//
// Возвращаемое значение:
//  Строка - контекст пользователя (ФИО + группы доступа)
Функция КонтекстТекущегоПользователя() Экспорт
	...
	Контекст = Символы.ПС + Символы.ПС + СтрШаблон(
		НСтр("ru = 'Пользователь, с которым вы общаетесь: %1'"), ИмяПользователя);

	СписокГрупп = ПолучитьГруппыПользователя(ТекущийПользователь);
	Если СписокГрупп.Количество() > 0 Тогда
		Контекст = Контекст + Символы.ПС + СтрШаблон(
			НСтр("ru = 'Группы доступа пользователя: %1'"), СтрСоединить(СписокГрупп, ", "));
	КонецЕсли;
...
// Перебирает возможные имена регистра, т.к. они различаются в разных конфигурациях.
//
// Параметры:
//  ТекущийПользователь - СправочникСсылка - пользователь
//
// Возвращаемое значение:
//  Массив - имена групп пользователя
	Функция ПолучитьГруппыПользователя(ТекущийПользователь)
		...
		ЗапросГрупп.Текст =
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			| Группы.Ссылка.Наименование КАК ИмяГруппы
			|ИЗ
			| Справочник.ГруппыДоступа.Пользователи КАК Группы
			|ГДЕ
			| Группы.Пользователь = &Пользователь";
		ЗапросГрупп.УстановитьПараметр("Пользователь", ТекущийПользователь);

		РезультатЗапроса = ЗапросГрупп.Выполнить();
		Если Не РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать();
			Пока Выборка.Следующий() Цикл
				СписокГрупп.Добавить(Выборка.ИмяГруппы);
			КонецЦикла;
		КонецЕсли;
